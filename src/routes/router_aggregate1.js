...
router.post('/count-courses-by-tags', asyncRoute(async (req, res) => {
  const result = await Course.aggregate([
...
router.post('/top-tags', asyncRoute(async (req, res) => {
  const result = await Course.aggregate([
    { $unwind: '$tags' },
    { $group: { _id: '$tags', count: { $sum: 1 } } },
    { $sort: { count: -1 } },
...
router.post('/top-courses-by-level', asyncRoute(async (req, res) => {
  const result = await Course.aggregate([
    { $unwind: '$level' },
...
router.post('/courses-filter', asyncRoute(async (req, res) => {
  const result = await Course.aggregate([
    { $match: { price: { $gte: 100000, $lte: 200000 } } },
    { $sort: { publishedAt: -1, _id: -1 } },
    { $limit: 10 },
    { $project: { _id: 1, title: 1, price: 1, level: 1, tags: 1, publishedAt: 1, stats: 1 } }
  ])
  res.json(ok(result, { message: 'DANH SÁCH KHÓA HỌC FILTER' }));
}))

router.post('/courses-filter-and-facet', asyncRoute(async (req, res) => {
...
router.post('/courses-filter-and-panigate', asyncRoute(async (req, res) => {
  const { min = 100000, max = 200000, limit = 10 } = req.body ?? {};
  const baseMatch = { price: { $gte: +min, $lte: +max } };
...
// Mục tiêu: dùng $facet để trả list + thống kê.
// Đề bài: POST / courses / filter + facets nhận { levels, tags, minPrice, maxPrice, limit } trả:

//   items[](limit),
//   byLevel[](group level),
//   topTags[](unwind + top 20),
//   totalCount.
// Gợi ý pipeline: $match → $sort → $facet{ items, byLevel, topTags, totalCount }.
// Nộp: pipeline + response mẫu(10 dòng), allowDiskUse(true) nếu cần.
//   PASS: các khối thống nhất với cùng filter; thời gian chạy ổn định khi limit thay đổi.

router.post('/courses-filter-and-facets', asyncRoute(async (req, res) => {
  const { min = 100000, max = 200000, limit = 10 } = req.body ?? {};
  const baseMatch = { price: { $gte: +min, $lte: +max } };
  ...
  res.json(ok(data, { message: 'DANH SÁCH KHÓA HỌC FILTER + FACETS', meta: { total } }));
